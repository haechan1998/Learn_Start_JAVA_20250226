...
public class Exception01 {
	...
	public static void main(String[] args) {
		...
		/* 
		 * 오류의 종류.
		 * - 논리적 오류 : 실행은 되지만 의도와는 다르게 동작하는 것.
		 * - 컴파일 에러 : 컴파일 시 발생하는 에러. (문법 구문 오류 : syntax error) > 
		 * - 런타임 에러 : 실행 시 잘못된 결과를 얻거나, 외부적인 요인으로 비정상 종료가 되는 경우.
		 * 
		 * 실행시(runTime) 발샐할 수 있는 오류를
		 ...
		 * - 예외처리 => 예외가 발생되는 코드만 빼고, 나머지는 정상 처리로 유지시키는 기능.\
		 * 
		 * - try ~ catch
		 * - try ~ catch ~ finally
		 * 
		 ...
		 * - finally : try 구문외 반드시 실행되어야 하는 구문을 작성 (생각가능)
		 * */
		...
		try {
			...
			System.out.println("0으로 나누었습니다."); // 직접 입력 가능.
			
			System.out.println(e.getMessage()); // getMessage 는 string 으로 리턴하기 때문에 syso에 넣어줘야한다.
		}
